...
(async()=>{
 ...
 async function setup(kind,saveFail=false){await page.evaluate(({kind,saveFail})=>{
  ...
  const p={zones:[{id:'test',name:'Test'}]};ensurePlanting=()=>p;plantRefsOn=()=>false;_adminKey=()=> 'test';_renderPrompt=()=> 'Test render';_zoneNames=()=>['Test plant'];_toast=()=>{};_plRender=()=>{};_zoneRenderAdd=()=>true;
 ...
 for(const kind of ['install','mature','custom','refine']){
  ...
  await page.locator('#palette-render-progress').waitFor();assert.equal(await page.locator('#palette-render-progress progress').getAttribute('value'),null);
  ...
  await page.evaluate(()=>window.renderTask);assert.equal(await page.locator('#palette-render-progress,#rp-modal,#rf-modal').count(),0);console.log(kind,'completed and both dialogs closed');
 }

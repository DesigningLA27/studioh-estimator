...
extra='''<dialog id="v2-projects"><h2>Open a project</h2><p>Open an existing Cloudflare project as a V2 working copy. Your original stays unchanged.</p><label>Import project JSON<input id="v2-import" type="file" accept=".json,application/json"></label><button id="v2-sample">Open SAMPLE DEMO</button><button id="v2-cloud-list">Browse saved projects</button><div id="v2-cloud-projects"></div><button id="v2-restore-previous">Restore previous working copy</button><button onclick="this.closest('dialog').close()">Close</button><p>Cloud projects can be read here. V2 changes are saved on this device; cloud write-back is not connected.</p></dialog>'''
mock=mock.replace('<main id="sh-content"></main></div></div>','<main id="sh-content"></main></div>'+extra+'</div>')
html=('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Studio H · V2 Preview</title><link rel="stylesheet" href="src/shell.css"></head><body>'+mock+'<div id="v2-notice" hidden role="status"></div><script src="src/shell.js?v=10"></script><link rel="stylesheet" href="src/workspace.css?v=10"><script src="src/icons.js"></script><link rel="stylesheet" href="src/libraries.css?v=10"><script src="src/libraries.js?v=10"></script><script src="src/workspace.js?v=10"></script><script src="src/experience.js"></script></body></html>')

// Projects embed their selected plant records, not a second copy of the shared master catalog.
_bidPlants=function(){const out={shrub:[],gc:[],tree:[],palm:[]},seen=new Set();for(const z of S.planting?.zones||[])for(const kind of Object.keys(out))for(const r of z[kind+'Rows']||[]){const key=kind+':'+r.pid;if(seen.has(key))continue;seen.add(key);const p=plantDbFind(kind,r.pid);if(p)out[kind].push(JSON.parse(JSON.stringify(p)))}return out};
...
(()=>{
 ...
 function save(){v2PublishIdentity();const bid=snapshot();if(!bid)throw Error('Project not ready');localStorage.setItem('v2_project',JSON.stringify(bid));send('snapshot',{bid});send('saved',{name:bid.S?.pi?.project||bid.S?.pi?.client||'Project'});return bid}
...
function v2ClearProjectCards(){v2ProjectIdentity();

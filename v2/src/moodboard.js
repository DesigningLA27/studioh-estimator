...
window.v2MoodboardDirection=function(){let d=document.getElementById('vmb-direction');if(!d){d=document.createElement('dialog');d.id='vmb-direction';d.className='vmb-dialog';document.body.append(d)}const mb=_mbEnsure();d.innerHTML=`<form method="dialog"><header><h2>Design direction</h2><button class="vmb-button" value="cancel">Close</button></header><label>Headline<input name="title" maxlength="120" value="${escape(mb.directionTitle||'')}" placeholder="Quiet geometry. Warm materials."></label><label>Design story<textarea name="notes" rows="4" maxlength="1000">${escape(mb.directionNotes||'')}</textarea></label><button class="vmb-button" value="save">Save direction</button></form>`;d.onclose=()=>{if(d.returnValue!=='save')return;const data=new FormData(d.querySelector('form'));mb.directionTitle=data.get('title').trim();mb.directionNotes=data.get('notes').trim();_bidSchedule();renderMoodBoard()};d.showModal()};

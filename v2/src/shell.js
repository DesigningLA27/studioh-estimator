...
(()=>{
 ...
 fetch('engine.html?v=5').then(r=>{if(!r.ok)throw Error('Engine could not load');return r.text()}).then(html=>{let seed={};try{seed=JSON.parse(localStorage.getItem(keys.store)||'{}')}catch{}const json=JSON.stringify(seed).replaceAll('<','\\u003c');frame.srcdoc=html.replace('/*V2_STORAGE_SEED*/{}',json)}).catch(e=>notice(e.message));

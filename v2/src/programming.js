...
function v2Programming(){
 ...
 host.innerHTML=`<section class="v2-trace-hero"><div><small>THIS PROJECT’S TRACE</small><h1>${has?escape(S.v2Programming?.name||S.pi?.project||'Your saved plan'):'Start with your site.'}</h1><p>${has?'Open your project’s current plan and continue tracing.':'Choose a PDF, satellite view or CAD drawing to begin. Your tracing stays with this project.'}</p><div class="v2-plan-facts"><span>${count} traced items</span><span>${has?(PT.pdfData?'PDF plan':'Satellite View'):'No plan yet'}</span><span>${PT.ppf?'Scale set':'Scale not set'}</span></div><div class="v2-program-actions">${has?'<button class="btn primary" data-action="resume">Open existing trace →</button>':'<button class="btn primary" data-action="pdf">Upload plan →</button>'}<button class="btn" data-action="demo">Open Sample Demo</button></div><details><summary>Restore from backup</summary><p>Open a Studio H trace backup (.json) containing shapes, scale and usually the original PDF. A project backup replaces the whole project; export your current work first.</p><button class="btn" data-action="restore">Choose backup file</button></details></div><div class="v2-trace-preview" id="v2-trace-preview"><span>${has?'Preparing plan preview…':'Your plan preview will appear here.'}</span></div></section>
 ...
 <article><span class="v2-source-symbol"><svg viewBox="0 0 64 64" aria-hidden="true"><path d="M14 5h25l12 12v38a4 4 0 0 1-4 4H14a4 4 0 0 1-4-4V9a4 4 0 0 1 4-4Z"/><path class="cut" d="M38 5v14h14"/><path class="cut-line" d="M20 29h23v20H20Zm0 10h13V29m0 10v10"/></svg></span><h2>Use a PDF or image</h2><p>Upload a PDF, JPG or PNG. Set the scale using a known dimension, then trace directly on the plan.</p><button class="btn" data-action="pdf">Upload plan →</button></article>
 <article><span class="v2-source-symbol"><svg viewBox="0 0 64 64" aria-hidden="true"><path d="m5 18 17-5 19 5 18-5v37l-18 5-19-5-17 5Z"/><path class="cut-line" d="M22 17v30m19-24v28"/><path d="M32 3c-9 0-15 6-15 14 0 10 15 23 15 23s15-13 15-23C47 9 41 3 32 3Z" stroke="var(--bg)" stroke-width="3"/><circle class="cut" cx="32" cy="17" r="5"/></svg></span><h2>Satellite View</h2><p>${escape(S.pi?.address)||'Add a property address in Project info.'}</p><p class="v2-source-note">Open the existing Studio H tracer over the property’s satellite image.</p><button class="btn" data-action="map">Open Satellite View →</button></article>
 ...
 <div id="v2-program-status" role="status"></div><input hidden id="v2-program-pdf" type="file" accept="application/pdf,.pdf,image/jpeg,image/png,.jpg,.jpeg,.png"><input hidden id="v2-program-cad" type="file" accept=".dxf,.dwg,.pdf"><input hidden id="v2-program-trace" type="file" accept="application/json,.json">`;
 ...
 const importPlan=async e=>{
  ...
  try{if(/\.dwg$/i.test(file.name))throw Error('DWG requires conversion. Export a PDF from your CAD app, then upload that PDF here.');const data=/\.dxf$/i.test(file.name)?await v2DxfPdf(await file.text()):(/\.(jpe?g|png)$/i.test(file.name)?await v2ImagePlan(file):await file.arrayBuffer());const ok=await _ptOpenDocBuf(data,false);if(ok){S.v2Programming={name:file.name};_bidSchedule();status.textContent='Plan opened in the tracer.'}else status.textContent='The plan could not be opened. Please try another PDF.';}catch(err){status.textContent='Could not open this plan: '+err.message}finally{e.target.value=''}
 };
...
ptClose=function(...args){document.getElementById('pt-scale-pop')?.remove();const result=v2OriginalTracerClose.apply(this,args);parent.postMessage({v2:'tracer-state',open:false},'*');if(document.getElementById('v2-programming')){v2Programming();_bidSchedule()}return result};
...
async function v2TraceThumbnail(host){
 const box=host.querySelector('#v2-trace-preview');if(!PT.pdfData){if(PT.satBase)v2SatelliteThumbnail(box);return}
...
async function v2ImagePlan(file){const url=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(file)});const img=new Image();img.src=url;await img.decode();const J=await _ptLoadJsPDF();const w=img.naturalWidth,h=img.naturalHeight,doc=new J({orientation:w>h?'landscape':'portrait',unit:'pt',format:[w,h]});doc.addImage(url,/png/i.test(file.type)?'PNG':'JPEG',0,0,w,h);return doc.output('arraybuffer')}
const v2OriginalRenderTracer=ptRenderModal;
ptRenderModal=function(...args){parent.postMessage({v2:'tracer-state',open:true},'*');return v2OriginalRenderTracer.apply(this,args)};
async function v2OpenActualTrace(){try{if(PT.satBase){for(let i=0;i<100&&!window.google?.maps?.geometry;i++)await new Promise(r=>setTimeout(r,200));if(!window.google?.maps?.geometry)throw Error('Google Maps could not load. Your saved trace is loaded; retry Open existing trace.')}await ptReopen()}catch(e){parent.postMessage({v2:'error',message:e.message},'*')}}

function v2SatelliteThumbnail(box){const sb=PT.satBase;if(!sb?.live){box.innerHTML='<span>Open the saved satellite trace to refresh its preview.</span>';return}const pts=PT.polys.flatMap(p=>p.pts||[]).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));const url=new URL(_satPlanUrl({center:sb.center,zoom:sb.zoom||20,paths:[]}));url.searchParams.set('size','640x480');if(pts.length){const ys=pts.map(p=>p.y),xs=pts.map(p=>p.x),south=Math.min(...ys),north=Math.max(...ys),west=Math.min(...xs),east=Math.max(...xs),merc=lat=>Math.log(Math.tan(Math.PI/4+lat*Math.PI/360))/(2*Math.PI);const zoom=Math.min(21,Math.floor(Math.log2(560/(256*Math.max((east-west)/360,1e-8)))),Math.floor(Math.log2(400/(256*Math.max(merc(north)-merc(south),1e-8)))));url.searchParams.set('center',`${(north+south)/2},${(west+east)/2}`);url.searchParams.set('zoom',zoom);for(const p of PT.polys){if(!p.pts?.length)continue;const color=({lot:'ffffff',planting:'55963b',grass:'7da33b',paving:'999999',pool:'66cde2',spa:'269ac4',building:'ffffff',driveway:'777777'})[p.mode]||'8baf66';const ring=p.kind==='linear'?p.pts:[...p.pts,p.pts[0]];url.searchParams.append('path',`color:0x${color}ff|weight:2|fillcolor:0x${color}${p.mode==='lot'?'00':'66'}|`+ring.map(q=>q.y.toFixed(6)+','+q.x.toFixed(6)).join('|'))}}if(url.href.length>8000)url.searchParams.delete('path');const img=new Image();img.alt='Current project satellite base and saved traced areas';img.src=url.href;img.style.cssText='display:block;width:100%;height:auto';img.onerror=()=>{box.innerHTML='<span>Satellite preview unavailable. Open existing trace to view the live map.</span>'};box.replaceChildren(img)}
